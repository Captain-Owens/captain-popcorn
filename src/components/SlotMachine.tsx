...
export default function SlotMachine({ isOpen, onClose, memberId, onWatch }: SlotMachineProps) {
  ...
  const totalReelHeight = reelPosters.length * (posterHeight + 12);
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-50 flex items-center justify-center bg-rich-black/90 px-4"
      ...
      >
        <motion.div
          ...
          className="w-full max-w-[400px] bg-charcoal rounded-card p-6 relative flex flex-col items-center"
        >
          <button onClick={onClose} className="absolute top-4 right-4 w-10 h-10 flex items-center justify-center text-muted hover:text-cream btn-press rounded-full">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
            </svg>
          </button>
          <h2 className="text-xl font-bold text-center mb-6 text-warm-gold">Feeling lucky?</h2>
          {spinning && (
            <div className="relative w-36 mx-auto mb-6 overflow-hidden rounded-card border-2 border-warm-gold" style={{ height: posterHeight }}>
              <div className="absolute inset-x-0 top-0 h-12 bg-gradient-to-b from-charcoal to-transparent z-10 pointer-events-none" />
              ...
              <motion.div
                ...
                transition={{ duration: SPIN_DURATION, ease: [0.2, 0.8, 0.3, 1] }}
              >
                {reelPosters.map((poster, i) => (
                  <div key={i} className="flex-shrink-0 rounded-btn overflow-hidden bg-smoke" style={{ width: 144, height: posterHeight }}>
          ...
          {!spinning && result && (
            <motion.div
              ...
              className="w-full"
            >
              <RecommendationCard rec={result} onWatch={(id) => { onWatch(id); onClose(); }} />
              <button onClick={spin} className="w-full mt-4 py-3 bg-warm-gold text-rich-black rounded-btn font-bold btn-press min-h-[48px]">Spin again</button>
            </motion.div>
          )}
          {!spinning && empty && (
