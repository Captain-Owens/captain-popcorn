  ...
  if (loading) {
    return (
      <div style={{ padding: '0 20px', marginBottom: 24 }}>
        <div style={{
          background: '#2a2a2e', borderRadius: 16, height: 320,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          color: '#888', fontSize: 14,
        }}>Finding movies for you...</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ marginBottom: 24, position: 'relative' }}>
      <div
        ...
        style={{ overflow: 'hidden', padding: '0 20px', touchAction: 'pan-y' }}
      >
        <div style={{
          display: 'flex',
              ...
              <div key={item.id} style={{ flex: '0 0 100%', minWidth: '100%' }}>
                <div style={{ background: '#2a2a2e', borderRadius: 16, overflow: 'hidden', position: 'relative' }}>
                  <div style={{ position: 'relative', width: '100%', aspectRatio: '2/3' }}>
                    {item.poster_url ? (
                      <Image src={item.poster_url} alt={item.title} fill
                        style={{ objectFit: 'cover' }} sizes="(max-width: 480px) 100vw, 440px" />
                    ) : (
                      <div style={{ width: '100%', height: '100%', background: '#1a1a1e',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        color: '#666', fontSize: 48 }}>🎬</div>
                    )}
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: isSaved ? 'rgba(218,165,32,0.95)' : 'rgba(0,0,0,0.6)',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                    ...
                    <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0,
                      ...
                      <h3 style={{ fontSize: 20, fontWeight: 700, color: '#fff',
                        ...
                        textShadow: '0 1px 4px rgba(0,0,0,0.5)' }}>{item.title}</h3>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8,
                        fontSize: 13, color: '#ccc' }}>
                        {item.year && <span>{item.year}</span>}
                        {item.tmdb_rating > 0 && <span style={{ color: '#DAA520' }}>★ {item.tmdb_rating.toFixed(1)}</span>}
